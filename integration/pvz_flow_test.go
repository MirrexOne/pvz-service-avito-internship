...
func TestMain(m *testing.M) {
	...
	testApp = app.NewApp(context.Background(), testConfig, testLogger, dbPool)
...
func TestPVZFullFlow(t *testing.T) {
	...
	assert.Equal(t, pvzID, createdReception.PvzId, "Reception PVZ ID mismatch")
	assert.Equal(t, api.InProgress, createdReception.Status, "Reception status should be in_progress")
	...
	for i := 0; i < 50; i++ {
		...
		assert.Equalf(t, receptionID, addedProduct.ReceptionId, "Product reception ID mismatch on iteration %d", i)
	...
	assert.Equal(t, receptionID, *closedReception.Id, "Closed reception ID mismatch")
