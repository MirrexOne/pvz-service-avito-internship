...
import (
	...
	"log" // Используем стандартный log, т.к. slog может быть еще не инициализирован на момент загрузки конфига
	"os"
	"time"
...
// Теги используются библиотекой cleanenv для маппинга значений.
type Config struct {
...
type Database struct {
	...
	SSLMode  string `yaml:"ssl_mode" env:"DB_SSL_MODE" env-default:"disable"` // Возможные значения: disable, require, prefer, verify-ca, verify-full
}

// Auth содержит настройки, связанные с аутентификацией и JWT.
type Auth struct {
	// JWTSecret - секретный ключ для подписи и верификации JWT токенов.
	JWTSecret string `yaml:"jwt_secret" env:"JWT_SECRET" env-required:"true"`
	// JWTttl - время жизни (Time To Live) для JWT токенов.
	// Формат: часы, минуты, секунды (например, "24h", "1h30m", "3600s").
...
type Logger struct {
	// Level - минимальный уровень логирования сообщений.
...
type Hasher struct {
	...
	// Чем выше значение, тем безопаснее, но медленнее. Рекомендуется 10-12.
...
func Load() *Config {
	...
	// Читаем переменные окружения (теперь они включают значения из .env, загруженные godotenv)
	...
	// На этом этапе все обязательные поля должны быть заполнены из окружения (включая .env)
...
func LoadTestConfig() *Config {
	...
	if port := os.Getenv("TEST_DB_PORT_HOST"); port != "" {
