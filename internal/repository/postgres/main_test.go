...
func runTestMigrations(log *slog.Logger, migrationsPath, databaseURL string) {
	...
	// Сначала пытаемся откатить все на случай "грязного" состояния
	...
	err = m.Up()
	sourceErr, dbErr := m.Close() // Закрываем соединения migrate
	if sourceErr != nil {
...
func clearTables(ctx context.Context, pool *pgxpool.Pool, tables ...string) error {
	...
	for _, table := range tables {
		...
		if _, err := tx.Exec(ctx, sql); err != nil {
			...
			// Не возвращаем ошибку сразу, пытаемся очистить остальные
		}
	}
