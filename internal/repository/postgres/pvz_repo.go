...
type PVZRepository struct {
	BaseRepository // Встраиваем базовый репозиторий
}
...
func (r *PVZRepository) GetByID(ctx context.Context, id uuid.UUID) (*domain.PVZ, error) {
	...
	if err != nil {
		return nil, r.wrapErr(op, err) // wrapErr обработает ErrNoRows
	}
	return &pvz, nil
}
...
func (r *PVZRepository) ListIDsAndTotal(ctx context.Context, limit, offset int, startDate, endDate *time.Time) ([]uuid.UUID, int, error) {
	...
	// --- Шаг 1: Подсчет общего количества ---
	...
	// --- Шаг 2: Получение ID ПВЗ для текущей страницы ---
	...
	if err != nil {
		...
		// wrapErr обработает ErrNoRows, если вдруг возникнет
...
func (r *PVZRepository) GetByIDs(ctx context.Context, ids []uuid.UUID) ([]domain.PVZ, error) {
	...
	if err != nil {
		// Используем функцию пакета isErrNoRows
		if isErrNoRows(err) {
...
func (r *PVZRepository) ListAll(ctx context.Context) ([]domain.PVZ, error) {
	...
	if err != nil {
		// Используем функцию пакета isErrNoRows
		if isErrNoRows(err) {
			return []domain.PVZ{}, nil // Если таблица пуста, возвращаем пустой срез
		}
		return nil, r.wrapErr(op, err) // Оборачиваем другие ошибки
	}
	defer rows.Close()
