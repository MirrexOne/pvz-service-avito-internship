...
import (
	...
	// Не импортируем pgx напрямую здесь, используем isErrNoRows из пакета
...
func (r *ReceptionRepository) ListByPVZIDsAndDate(ctx context.Context, pvzIDs []uuid.UUID, startDate, endDate *time.Time) (map[uuid.UUID][]domain.ReceptionWithProducts, error) {
	...
	// --- Шаг 1: Получаем все релевантные приемки ---
	...
	// --- Шаг 2: Получаем все товары для найденных приемок ---
	productRepo := NewProductRepository(r.db, r.log) // Создаем здесь, т.к. это вспомогательный метод
	...
	// --- Шаг 3: Собираем финальную структуру ---
