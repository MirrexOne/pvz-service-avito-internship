...
// УБИРАЕМ локальный хелпер createTestReception
...
func TestReceptionRepository_Create_GetByID(t *testing.T) {
	...
	t.Run("Create with Non-existent PVZ ID", func(t *testing.T) {
		reception := domain.Reception{
			...
			PVZID:    uuid.New(), // Несуществующий PVZ ID
			Status:   domain.StatusInProgress,
		}
		errCreate := repo.Create(ctx, &reception)
		require.Error(t, errCreate)
		// Ожидаем ошибку нарушения внешнего ключа (FK)
...
func TestReceptionRepository_FindOpenByPVZID(t *testing.T) {
	...
	// Создаем приемки с помощью глобального хелпера
	rec1Open := createTestReception(ctx, t, repo, pvz1.ID, time.Now().Add(-time.Hour))     // Открытая
	rec1Closed := createTestReception(ctx, t, repo, pvz1.ID, time.Now().Add(-2*time.Hour)) // Закрытая
	errUpdate := repo.UpdateStatus(ctx, rec1Closed.ID, domain.StatusClosed)
	require.NoError(t, errUpdate)
	// Для pvz2 нет открытых
