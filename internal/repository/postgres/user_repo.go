...
// Обрабатывает ошибку уникальности email и возвращает domain.ErrConflict.
func (r *UserRepository) Create(ctx context.Context, user *domain.User) error {
	const op = "UserRepository.Create"

	query, args, err := r.sq.Insert("users").
		Columns("id", "email", "password_hash", "role").
		Values(user.ID, user.Email, user.PasswordHash, user.Role). // Используем PasswordHash
		ToSql()
	if err != nil {
...
func (r *UserRepository) GetByEmail(ctx context.Context, email string) (*domain.User, error) {
	...
	query, args, err := r.sq.Select("id", "email", "password_hash", "role").
		From("users").
		Where(sq.Eq{"email": email}). // Ищем по email
		Limit(1).
		ToSql()
	if err != nil {
	...
	if err != nil {
		return nil, r.wrapErr(op, err) // wrapErr обработает ErrNoRows -> ErrNotFound
	}
	return &user, nil
}
