...
func TestUserRepository_Create(t *testing.T) {
	...
	newUser := &domain.User{
		...
		PasswordHash: "some_hash", // Хеш не проверяется здесь
	...
	t.Run("Success", func(t *testing.T) {
		...
		// Проверяем, что пользователь действительно создан (опционально, но надежно)
	...
	t.Run("Fail_Duplicate_Email", func(t *testing.T) {
		duplicateUser := &domain.User{
			ID:           uuid.New(),
			Email:        "create.test@example.com", // Тот же email
...
func TestUserRepository_GetByEmail(t *testing.T) {
	...
	// Очищаем и создаем тестового пользователя
	err := clearTables(ctx, dbPool, "users")
	require.NoError(t, err)
