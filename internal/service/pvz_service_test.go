...
func TestPVZService_ListPVZs(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "Success_Empty_Page", // Запросили страницу, на которой нет ПВЗ
			limit: testLimit,
			page:  2, // За пределами testTotal при limit=10
			setupMocks: func() {
				offsetPage2 := (2 - 1) * testLimit
				mockPVZRepo.On("ListIDsAndTotal", ctx, testLimit, offsetPage2, (*time.Time)(nil), (*time.Time)(nil)).
					Return([]uuid.UUID{}, testTotal, nil).Once() // Возвращает пустой слайс ID
				// GetByIDs и ListByPVZIDsAndDate не должны вызываться
			},
		...
		{
			name:  "Success_No_PVZs_Found", // Фильтр не нашел ни одного ПВЗ
			limit: testLimit,
			page:  testPage,
			setupMocks: func() {
				mockPVZRepo.On("ListIDsAndTotal", ctx, testLimit, testOffset, (*time.Time)(nil), (*time.Time)(nil)).
					Return([]uuid.UUID{}, 0, nil).Once() // Возвращает total = 0
				// GetByIDs и ListByPVZIDsAndDate не должны вызываться
			},
			expectedResultCount: 0,
			expectedTotal:       0,
		...
		},
		{
			...
			setupMocks: func() {
				...
				// ListByPVZIDsAndDate не должен вызываться
			},
			expectedResultCount: 0,
			expectedTotal:       0,
			expectedError:       domain.ErrDatabaseError,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedError != nil {
				...
				if len(results) > 0 {
					assert.Equal(t, testPVZs[0].ID, results[0].PVZ.ID) // Проверяем порядок
					if tc.name == "Success_No_Filters" {
						assert.Len(t, results[0].Receptions, 1) // Для pvzID1
						assert.Len(t, results[1].Receptions, 0) // Для pvzID2
					}
				}
			}
